<?php
  session_start();
  $path = $_SERVER['DOCUMENT_ROOT']."/WIMF-master/code/";
...
<script src = "/WIMF-master/code/script/addElement.js"></script>
<?php
      ...
      <?php
        if ( isset( $_SESSION[ "errorsForm" ] ) ) {
          foreach( $_SESSION[ "errorsForm" ] as $keyError )
            echo "<li style='color:red'>".$_SESSION[ "errorsForm" ][ $keyerror ]."</li>";
          unset( $_SESSION[ "errorsForm" ] );
        }
      ?>

      <form method="POST" action="/WIMF-master/code/script/saveRecipe.php" id="formRecipe">
